...
class TextUtils:
    ...
    def read_words(filename):
        with open(filename, 'r', encoding= "utf-8") as file:
            return [sentence.strip().split() for sentence in set(file.readlines())]
...
class MultiWord2VecModel():
    def __init__(self, size=300, window=1, min_count=30, workers=4, sg=1, sample=1e-5, iter=10,
                 output_dirpath='embedding/output/'):
        self.size = size
        self.window = window # window size
        self.min_count = min_count
...
class ReplacingWord2VecModel() :
    ...
    def replace(self, word):
        if word in self.model.wv:
            ...
            pos = word.split('/')[1]
            candidates = [vocab_word for vocab_word in self.vocab if vocab_word.endswith(pos)]
            if candidates:
                replace_word = random.choice(candidates)
            else:
                replace_word = random.choice(list(self.vocab))
